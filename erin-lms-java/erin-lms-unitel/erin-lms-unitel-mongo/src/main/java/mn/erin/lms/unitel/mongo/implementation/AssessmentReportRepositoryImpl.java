...
import mn.erin.lms.unitel.mongo.document.MongoAssessmentAnswer;
import mn.erin.lms.unitel.mongo.document.MongoAssessmentReport;
import mn.erin.lms.unitel.mongo.document.MongoAssessmentReportItem;
...
public class AssessmentReportRepositoryImpl implements AssessmentReportRepository
{
  ...
  @Override
  public List<AssessmentReport> findAll(Set<String> ids)
  {
    Iterable<MongoAssessmentReport> result = mongoAssessmentReportRepository.findAllById(ids);
    Iterator<MongoAssessmentReport> iterator = result.iterator();

    List<AssessmentReport> assessmentReports = new ArrayList<>();

    while (iterator.hasNext())
    {
      assessmentReports.add(convert(iterator.next()));
    }
