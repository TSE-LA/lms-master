...
public class LearnerSuccessRepositoryImpl implements LearnerSuccessRepository
{
  ...
  @Override
  public List<LearnerSuccess> getSuccesses(LearnerId learnerId, String courseType, int year, boolean isFirstHalf)
  {
    List<MongoLearnerSuccess> learnerSuccesses = mongoLearnerSuccessRepository
        .getAllByLearnerIdAndCourseTypeAndYearAndMonthBetween(learnerId.getId(), courseType, year, isFirstHalf ? 0 : 6, isFirstHalf ? 7 : 13);
  ...
  @Override
  public boolean isExistThisMonth(int year, int month, String courseType)
  {
    return mongoLearnerSuccessRepository.countByYearAndMonthAndCourseType(year, month, courseType) > 0;
  }
