...
public class DmsSCORMWrapperRepository extends DmsSCORMRepository implements SCORMWrapperRepository
{
  ...
  public DmsSCORMWrapperRepository(DocumentRepository documentRepository, FolderRepository folderRepository)
  {
  ...
  {
    Folder defaultWrapperFolder;
    try
    {
      defaultWrapperFolder = folderRepository.getFolder(getBaseWrapperFolder().getFolderId(), DmsSCORMConstants.DEFAULT_WRAPPER_FOLDER);
    }
    catch (DMSRepositoryException e)
    {
      LOGGER.error(e.getMessage(), e);
      throw new SCORMRepositoryException("Default wrapper folder was not found!", e);
    }

    return getWrapper(defaultWrapperFolder.getFolderId(), DmsSCORMConstants.DEFAULT_WRAPPER_FOLDER);
  }

  @Override
  public SCORMWrapper getWrapper(String type) throws SCORMRepositoryException
  {
    if (DmsSCORMConstants.DEFAULT_WRAPPER_FOLDER.equalsIgnoreCase(type))
    {
