...
public class GenerateSurveyExcelReport implements UseCase<GetCourseAnalyticsInput, byte[]>
{
  private final CourseAnalyticsService courseAnalyticsService;

  public GenerateSurveyExcelReport(CourseAnalyticsService courseAnalyticsService)
  {
  ...
  public byte[] execute(GetCourseAnalyticsInput input) throws UseCaseException
  {
    ...
    return courseAnalyticsService.generateSurveyAnalyticsData(courseId, departmentId, input.getStartDate(), input.getEndDate(), input.getUserId());
  }
}
