...
import mn.erin.lms.unitel.domain.usecase.dto.LearnerSuccessDto;
import mn.erin.lms.unitel.domain.usecase.dto.LearnerSuccessInput;
import mn.erin.lms.unitel.domain.usecase.dto.LearnerSuccessMonthData;

/**
 * @author Byambajav
 */
public class GetLearnerSuccess implements UseCase<LearnerSuccessInput, LearnerSuccessDto>
{
  ...
  @Override
  public LearnerSuccessDto execute(LearnerSuccessInput input) throws UseCaseException
  {
    ...
    List<LearnerSuccess> learnerSuccesses = learnerSuccessRepository.getSuccesses(LearnerId.valueOf(input.getLearnerId()), input.getCourseType(), input.getYear(), isFirstHalf);

    List<LearnerSuccess> previousLearnerSuccesses = learnerSuccessRepository.getSuccesses(
        LearnerId.valueOf(input.getLearnerId()), input.getCourseType(), isFirstHalf ? input.getYear() - 1 : input.getYear(), !isFirstHalf
    );
