...
import mn.erin.lms.base.mongo.document.enrollment.EnrolledGroup;
...
import mn.erin.lms.jarvis.mongo.document.report.MongoReportData;
import mn.erin.lms.jarvis.mongo.repository.MongoCourseReportRepository;
import static mn.erin.lms.jarvis.domain.report.usecase.CourseAnalyticDataConverter.convert;
    ...
    if (course.getCourseContentId() != null)
    {
      ...
      try
      {
        ...
        for (Map.Entry<String, List<RuntimeData>> entry : sortedByUser.entrySet())
        {
          ...
          if (data.getStatus() > 0)
          {
            repeatedViewersCount += data.getInteractionsCount() >= 2 ? 1 : 0;
