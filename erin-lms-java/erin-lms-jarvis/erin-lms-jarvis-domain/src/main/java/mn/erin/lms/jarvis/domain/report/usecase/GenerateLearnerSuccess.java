...
public class GenerateLearnerSuccess implements UseCase<LocalDate, Void>
{
  ...
  {
    if (learnerSuccessRepository.isExistThisMonth(startDate.getYear(), startDate.getMonthValue()))
    {
    ...
    for (String learner : allLearners)
    {
      ...
      learnerSuccessRepository
          .save(new LearnerSuccess(learner, COURSE_TYPE, score.getScore(), score.getMaxScore(), performance, startDate.getYear(), startDate.getMonthValue()));
    }
    return null;
  }
